...
The builder publishes the privacy-safe `pin_last_updated` ISO calendar date plus four Boolean completeness flags on each matched `unit_pin_statuses` row: `pin_status_complete`, `pin_contact_complete`, `pin_meeting_complete`, and `pin_details_complete`. Required PIN Details is true only when status, contact name plus either email or phone, meeting location, and meeting details are present. Website, fee, fundraising, and availability are not counted, and freshness remains a separate PIN Currency measure. The PIN Status & Completeness page divides complete matched rows by all tracked units in the selected district/program view, including unmatched units. Do not publish the raw source timestamp or the underlying contact or meeting values; only the normalized date and Boolean flags are allowed in public JSON.

District PIN Detail joins those privacy-safe PIN rows back to the complete unit-level population by normalized district and unit identity. Its expanded district unit rows inherit the active program, Service Area, District, and follow-up focus; sort unmatched, Stale, Inactive, incomplete, and complete Active units in that order; and show PIN Status, Last Updated, and Required PIN Details in that order before the missing categories. Last Updated uses the normalized calendar date, `Not recorded` for a matched PIN without a usable date, and `n/a` for an unmatched unit. Unit links pass the public `unit_id` query parameter so Unit-Level Detail selects the intended record.

Nested PIN unit headings track the measured district-header bottom when the outer table scrolls, and remain sticky within their own bounded scrollport. Keep `syncUnitHeaders`, resize observation, and the scoped header stacking rules together. Verify in Safari with the district row and the original unit-header position scrolled above the outer viewport; unit values must never appear directly beneath district-only headings. This is a display correction, not a PIN classification or completeness change.
...
Preserve `syncOperationalUnitHeaders`, the measured sticky-header offset, resize observation, and the scoped `operational-table` stacking rules. Exercise both scroll levels in Safari with the district summary row above the visible area: unit headings must remain immediately below the district headings. Recheck service-area expansion, district expansion, and program/status changes after any alteration.

Overview's Signals to Watch groups filtered matched rows into Active, Inactive, and Stale, derives unmatched as tracked units minus matched rows, and shows PIN Currency using the same denominator. The Overview Explore panel links directly to `pin-status.html`. Unit-Level Detail stores the complete matched `unit_pin_statuses` row. Unit Profile renders PIN status and freshness, Last Updated immediately below it, overall Required PIN Details, and separate status, contact, and meeting completion indicators. Last Updated uses the published `pin_last_updated` calendar date, `Not recorded` for a matched PIN without a usable date, and `n/a` for an unmatched unit. Do not infer completeness from freshness, combine the category flags, or expose the raw timestamp or underlying private values.
...
The page must retain the master program filter, Service Area and District filters, Status, missing-role Focus, search, overall KPIs, summary by unit type, and direct Unit-Level Detail links. Status offers All statuses, Complete, Missing 1, Missing 2, Missing 3, 1 or more expired SYTs, and 1 or more SYTs expiring within 90 days. The SYT choices evaluate every displayed holder against the viewer's current date, and all choices combine with every other filter. Unit Key 3 Detail uses a collapsible Service Area → District hierarchy, with missing-first unit rows inside each expanded district and page-level Expand all and Collapse all controls. Search results expand automatically. Beneath each displayed holder name, show the SYT expiration date instead of the holder's position title. Render dates due within 90 days of the viewer's current date in yellow and render expired dates in red; an unavailable date remains a neutral unavailable label and does not change Key 3 coverage. Each missing position must be explicit. Public holder names are first name plus last initial, and `unit_key3_statuses` must never contain email, phone, address, meeting, or other contact values.

Preserve the page route, navigation hierarchy, parent link, cache-busted assets, two-level disclosure controls, bounded scrolling unit tables, Help entry, Markdown documentation, generated reader guide, and structural validator together. Run `tools/test_unit_key3_statuses.py`, `tools/test_key3_status_page.js`, the public-name sanitizer tests, and `tools/validate_site_structure.py --require-data` against a sanitized staged snapshot before publication.
...
- District Performance → PIN Status & Completeness shows PIN status, currency, and Required PIN Details by district. Required PIN Details equals matched rows with all essential completion flags divided by all tracked units; the public JSON contains only the privacy-safe last-updated calendar date and Boolean completion flags, with no raw timestamp or PIN contact or meeting values. Expand a district to verify the individual-unit PIN Status, Last Updated, and Required PIN Details column order and the Unit-Level Detail links under every follow-up focus.
...
- Unit-Level Detail → Unit Profile shows PIN status and freshness, Last Updated immediately below it, overall Required PIN Details, and separate status, contact, and meeting completion indicators from the matched privacy-safe row. Verify a formatted calendar date, `Not recorded` for a matched PIN without a usable date, and `n/a` for an unmatched unit.
...
- Unit Key 3 Detail remains grouped in a collapsible Service Area → District hierarchy. Status offers All statuses, Complete, Missing 1, Missing 2, Missing 3, 1 or more expired SYTs, and 1 or more SYTs expiring within 90 days and combines with program, Service Area, District, Focus, and Search. Expand/collapse controls retain the filtered population, and expanded district tables list missing units first while preserving direct Unit-Level Detail links. The subtitle beneath each holder is their SYT expiration; dates due within 90 days are yellow and expired dates are red.
