...
`key3-status.html` is a child of **Unit Health & Renewal**. `work/commissioner_site/build_site.py` must publish `dashboard.unit_key3_statuses` with exactly one row for every tracked unit in the complete Unit Metrics population. Packs, Troops, Crews, and Ships evaluate three roles from the Training roster: the program-specific Unit Leader, Committee Chair, and the combined COR / CUR position. Either a current COR or CUR satisfies the third position. Explorer Posts require only a current Post Advisor and Committee Chair; COR/CUR is optional and must not create a missing role or lower Post completion. Publish the applicable `required_roles` on every row. A registration is current only when its expiration date is on or after the dashboard report date; a blank, unusable, or earlier expiration does not count.

The page must retain the master program filter, Service Area and District filters, missing-role focus, search, overall KPIs, summary by unit type, and direct Unit-Level Detail links. Unit Key 3 Detail uses a collapsible Service Area → District hierarchy, with missing-first unit rows inside each expanded district and page-level Expand all and Collapse all controls. Search results expand automatically. Beneath each displayed holder name, show the SYT expiration date instead of the holder's position title. Render the date red when it is expired or due within 90 days of the viewer's current date; an unavailable date remains a neutral unavailable label and does not change Key 3 coverage. Each missing position must be explicit. Public holder names are first name plus last initial, and `unit_key3_statuses` must never contain email, phone, address, meeting, or other contact values.
...
- Unit Health & Renewal → Unit Key 3 Coverage loads one `dashboard.unit_key3_statuses` row per tracked unit, treats either a current COR or CUR as satisfying the third position for Packs, Troops, Crews, and Ships, and applies the two-role Post Advisor plus Committee Chair rule to Explorer Posts. It clearly labels each required missing role and reconciles the overall totals with the summary by unit type.
- Unit Key 3 Detail remains grouped in a collapsible Service Area → District hierarchy. Expand/collapse controls retain the filtered population, and expanded district tables list missing units first while preserving direct Unit-Level Detail links. The subtitle beneath each holder is their SYT expiration; expired dates and dates due within 90 days are red.
