...
- Keep the Unit Health Funnel's `Inactive + stale PINs` measure on the complete `dashboard.unit_pin_statuses` dataset. Count both states together, filter by the master program view, and divide by all units in that view for the displayed rate.
