...
| `unit_pin_statuses` | Matched unit-level BeAScout PIN display states, the privacy-safe `pin_last_updated` ISO calendar date (or `null` when the source date is blank or unusable), plus `pin_status_complete`, `pin_contact_complete`, `pin_meeting_complete`, and `pin_details_complete` Boolean flags. Unmatched units are absent and display as `n/a` after the page join. The raw timestamp, contact names, email addresses, phone numbers, meeting locations, and meeting details are never published in this array. |
...
### District Operational Detail drill-down

On `districts.html`, Service Area rows continue to expand into district scorecard rows. Each district row can then expand into the complete individual-unit population for that district and the active master program view. Unit rows are ordered by unit metric from lowest to highest so priority attention appears first, then by unit name. They show the unit-health band, youth, calculated year-over-year rate, workbook metric, workbook retention percentage, matched PIN display state, commissioner-assignment presence, unit SYT and all-leader training rates, and a direct Unit-Level Detail link. No private PIN contact or meeting values are used or displayed.

### PIN Status & Completeness page

`pin-status.html` is a District Performance child page. It combines the shared PIN display-state classification with the Required PIN Details metric, supports the master program filter plus Service Area and District filters, and offers follow-up focuses for Stale, Inactive, details gaps, and no matched PIN. Every displayed percentage uses all tracked units in the selected view as its denominator. District PIN Detail rows expand to privacy-safe individual-unit rows showing program type, PIN Status, Last Updated, Required PIN Details state, missing categories, and a Unit-Level Detail link. Last Updated is the normalized `pin_last_updated` calendar date, appears between PIN Status and Required PIN Details, displays `Not recorded` for a matched PIN with a blank or unusable date, and displays `n/a` for an unmatched unit. Expanded units follow the active filters and are prioritized as unmatched, Stale, Inactive, incomplete, then complete Active. The page does not expose the raw timestamp or the private source values used to calculate the completion flags.
...
The page supports the master program filter plus Service Area, District, Status, missing-role Focus, and search. Status offers All statuses, Complete, Missing 1, Missing 2, Missing 3, 1 or more expired SYTs, and 1 or more SYTs expiring within 90 days. The two SYT choices evaluate every displayed holder against the viewer's current date, and all Status choices combine with the other filters. It shows overall KPIs, a summary by unit type, and missing-first unit detail grouped into collapsible Service Area and District levels with direct Unit-Level Detail links. Each holder object contains the privacy-safe `name`, qualifying `position`, and `syt_expires` date. The detail display replaces the position subtitle with the SYT expiration date; dates due within 90 days are yellow and expired dates are red, based on the viewer's current date. A missing or unusable SYT date displays as unavailable without changing Key 3 coverage. Holder names are published only as first name plus last initial and the Key 3 dataset contains no email, phone, address, or meeting fields.
...
The page also matches the selected unit to `dashboard.unit_pin_statuses` in `data/latest.json` and displays the result in the first KPI card. The card uses the same PIN display state as Unit Follow-up: `Stale` when more than 12 months have passed since the matched PIN's last update, or its update date is blank or unusable; otherwise it displays `Active` or `Inactive`. An unmatched unit displays `n/a`. Unit Profile repeats the status and freshness explanation, places Last Updated directly below it, shows overall Required PIN Details (`Details complete`, `Details need follow-up`, `Details unavailable`, or `Details n/a`), and details the privacy-safe status, contact, and meeting completion categories as `Complete`, `Needs follow-up`, `Unavailable`, or `n/a`. Last Updated comes directly from `pin_last_updated`, displays the formatted calendar date for a matched PIN, `Not recorded` when its date is blank or unusable, and `n/a` when no PIN is matched. The completion indicators come directly from the matched `pin_status_complete`, `pin_contact_complete`, `pin_meeting_complete`, and `pin_details_complete` flags. No raw timestamp or underlying contact or meeting value is published.

PIN data is refreshed daily from the `Pin` tab inside the same dated OneDrive `Dashboard - CAC.xlsx` workbook used to build `data/latest.json`; it is not sourced from the separate Unit Level Metrics workbook. Publication fails when the staged Council/PIN and Unit-Level bundles have different report dates, when PIN rows are empty despite a nonempty Unit-Level population, or when PIN rows are duplicate, extraneous, malformed, or contain fields beyond the approved privacy-safe schema. This validation does not turn a legitimate unmatched unit into a matched record; unmatched units remain `n/a`.
