...
| `unit_key3_statuses` | One row per tracked unit with its applicable required roles, current role coverage, missing-role indicators, and public first-name/last-initial holder labels. Packs, Troops, Crews, and Ships require Unit Leader, Committee Chair, and COR / CUR; Explorer Posts require only Post Advisor and Committee Chair. No contact fields are published. |
...
`key3-status.html` is a Unit Health & Renewal child page. It reads `dashboard.unit_key3_statuses`, which contains one row for every tracked unit. Packs, Troops, Crews, and Ships are evaluated for three roles: the program-specific Unit Leader, Committee Chair, and COR / CUR. Either a Chartered Organization Representative or Council Unit Representative satisfies the third Key 3 role. Explorer Posts require only a current Post Advisor and Committee Chair; COR/CUR is not required for Post completion. Each row publishes `required_roles` so the applicable denominator is explicit. A holder counts only when the Training roster position matches the role and the registration expiration is on or after the dashboard report date.

The page supports the master program filter plus Service Area, District, missing-role focus, and search. It shows overall KPIs, a summary by unit type, and missing-first unit detail grouped into collapsible Service Area and District levels with direct Unit-Level Detail links. Each holder object contains the privacy-safe `name`, qualifying `position`, and `syt_expires` date. The detail display replaces the position subtitle with the SYT expiration date; the date is red when expired or due within 90 days of the viewer's current date. A missing or unusable SYT date displays as unavailable without changing Key 3 coverage. Holder names are published only as first name plus last initial and the Key 3 dataset contains no email, phone, address, or meeting fields.
