...
def build_doc():
    ...
    add_table(
        ...
        [
            ...
            ["Unit Key 3 Coverage", "Which units have their applicable current Key 3 coverage, which positions are missing, and how completion compares by unit type. Packs, Troops, Crews, and Ships require Unit Leader, Committee Chair, and a third position; either a current COR or CUR satisfies that third position. Explorer Posts require only a current Post Advisor and Committee Chair. Detail is organized in collapsible Service Area and District groups and shows SYT expiration beneath each holder, with expired or 90-day dates in red."],
    ...
    add_table(
        ...
        [
            ...
            ["Is BALOO recorded?", "BALOO is shown as a person-level qualification for Pack leaders. Missing BALOO is not an individual issue; Pack-level depth is evaluated on Pack Camping Readiness."],
            ["Is IOLS missing?", "Scoutmaster and Assistant Scoutmaster rows use explicit IOLS status when available and otherwise are flagged when mandatory code S11 remains present."],
            ["What about all-leader views?", "Existing safety dates are shown for all rows. SYT issues apply to all leaders; Hazardous Weather and IOLS issues are evaluated where role and unit type make them applicable."],
        ],
        [2800, 6560],
    )
    add_body(
