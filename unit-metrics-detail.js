...
function renderControls() {
  ...
  const programSection = ProgramFilter.isCouncil() ? "All Units" : ProgramFilter.getType();
  if (sections.includes(programSection)) document.getElementById("sectionSelect").value = programSection;
  document.getElementById("sectionSelect").disabled = !ProgramFilter.isCouncil();
...
function renderKpis() {
  ...
  const tiles = [
    ...
    ["Retention", pWhole(summary.retentionRate), "Rounded to nearest whole percent; may exceed 100%", "teal"],
  ];
...
function renderRows() {
  ...
  document.getElementById("metricRows").innerHTML = serviceAreaGroups(rows).map((service) => {
    ...
    <tr class="service-area-row">
      ...
      <td class="num">${pWhole(summary.retentionRate)}</td>
    </tr>`;
