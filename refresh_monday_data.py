...
BOARDS = {
    ...
    "popcorn": {
        ...
        "columns": {
            "district": "color_mm33ta6a",
            "commitment": "color_mm2x2pfa",
            "sales_2025": "numeric_mm3gva8w",
            "goal_2026": "numeric_mm3ga9v5",
            "onboarding": "dropdown_mm3gj57e",
            "scouts_selling_2026": "text_mm3ga9q6",
            "scouts_selling_2025": "text_mm3g52fk",
            "onboarding_complete": "boolean_mm3gjypd",
            "budget_submitted": "date_mm3gqcbt",
            "kernel_recruited": "boolean_mm3gxg0k",
            "unit_trained": "boolean_mm3g5kht",
            "kickoff": "date_mm3gk397",
            "kickoff_photos": "boolean_mm3gpj6v",
            "bonus_1_pct": "boolean_mm3gbrd8",
            "bonus_2_pct": "boolean_mm3gstyd",
            "bonus_5_pct": "boolean_mm3gshhd",
        },
    },
}
...
POPCORN_ELIGIBLE_DISTRICT_KEYS = frozenset(
    district.casefold() for district in POPCORN_ELIGIBLE_DISTRICTS
)
POPCORN_COMMITMENT_COLUMNS = (
    "2026 Commitment Status",
    "Committed",
)
...
def is_checked(value: Any) -> bool:
    return as_text(value).lower() in {"v", "yes", "true", "1", "checked"}
...
def workbook_rows(workbook: Any, *sheet_names: str) -> list[dict[str, Any]]:
    sheet_name = next((name for name in sheet_names if name in workbook.sheetnames), None)
    if sheet_name is None:
        expected = ", ".join(sheet_names)
        raise RuntimeError(f"Workbook is missing required sheet: {expected}")
...
def is_eligible_popcorn_unit(name: Any, district: Any) -> bool:
    """Limit Popcorn reporting to non-Post units in an eligible district."""
    normalized_name = as_text(name).casefold()
    normalized_district = as_text(district).casefold()
    is_post = normalized_name == "post" or normalized_name.startswith(("post ", "post#"))
    return not is_post and normalized_district in POPCORN_ELIGIBLE_DISTRICT_KEYS


def validate_popcorn_commitments(rows: list[dict[str, Any]]) -> None:
    if not rows:
        raise RuntimeError("Popcorn commitment validation failed (no eligible unit rows)")
    invalid_statuses = Counter(
        row["commitment"]
        for row in rows
        if row["commitment"] not in POPCORN_COMMITMENT_STATUSES
    )
    if invalid_statuses:
        detail = ", ".join(
            f"{label}: {count}"
            for label, count in sorted(invalid_statuses.items())
        )
        raise RuntimeError(f"Popcorn commitment validation failed ({detail})")


def compact_popcorn_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Keep public, operational Popcorn fields; intentionally omit contact PII."""
    if rows and not any(
        any(column in row for column in POPCORN_COMMITMENT_COLUMNS)
        for row in rows
    ):
        expected = " or ".join(repr(column) for column in POPCORN_COMMITMENT_COLUMNS)
        raise RuntimeError(
            f"Popcorn workbook is missing the commitment column; expected {expected}"
        )
    ...
    compact_rows = [{
        ...
        "commitment": next(
            (
                as_text(row.get(column))
                for column in POPCORN_COMMITMENT_COLUMNS
                if as_text(row.get(column))
            ),
            "Unlabeled",
        ),
        "sales_2025": as_number(row.get("2025 Sales")),
        "goal_2026": as_number(row.get("2026 Goal")),
        "onboarding": as_text(row.get("Onboarding")) or "0/11",
        "scouts_selling_2026": as_text(row.get("2026 Scouts Selling")),
        "scouts_selling_2025": as_text(row.get("2025 Scouts Selling")),
        "onboarding_complete": is_checked(row.get("Unit Onboarding Completed")),
        "budget_submitted": excel_timestamp(row.get("Budget Worksheet Submitted")),
        "kernel_recruited": is_checked(row.get("Unit Kernel Recruited")),
        "unit_trained": is_checked(row.get("Unit Trained")),
        "kickoff": excel_timestamp(row.get("Popcorn Kickoff")),
        "kickoff_photos": is_checked(row.get("Kickoff Photos Submitted")),
        "bonus_1_pct": is_checked(row.get("1% Bonus")),
        "bonus_2_pct": is_checked(row.get("2% Bonus")),
        "bonus_5_pct": is_checked(row.get("5% Bonus")),
        "updated_at": excel_timestamp(row.get("Updated At")),
    } for row in rows if is_eligible_popcorn_unit(row.get("Item Name"), row.get("District"))]
    validate_popcorn_commitments(compact_rows)
    return compact_rows


def compact_popcorn_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    columns = BOARDS["popcorn"]["columns"]
    rows: list[dict[str, Any]] = []
    for item in items:
        values = column_map(item)
        if not is_eligible_popcorn_unit(item.get("name"), values.get(columns["district"])):
            continue
        rows.append({
            "item_id": as_text(item.get("id")),
            "name": as_text(item.get("name")),
            "group": "",
            "district": values.get(columns["district"]) or "Unassigned",
            "commitment": values.get(columns["commitment"]) or "Unlabeled",
            "sales_2025": as_number(values.get(columns["sales_2025"])),
            "goal_2026": as_number(values.get(columns["goal_2026"])),
            "onboarding": values.get(columns["onboarding"]) or "0/11",
            "scouts_selling_2026": values.get(columns["scouts_selling_2026"]) or "",
            "scouts_selling_2025": values.get(columns["scouts_selling_2025"]) or "",
            "onboarding_complete": is_checked(values.get(columns["onboarding_complete"])),
            "budget_submitted": values.get(columns["budget_submitted"]) or None,
            "kernel_recruited": is_checked(values.get(columns["kernel_recruited"])),
            "unit_trained": is_checked(values.get(columns["unit_trained"])),
            "kickoff": values.get(columns["kickoff"]) or None,
            "kickoff_photos": is_checked(values.get(columns["kickoff_photos"])),
            "bonus_1_pct": is_checked(values.get(columns["bonus_1_pct"])),
            "bonus_2_pct": is_checked(values.get(columns["bonus_2_pct"])),
            "bonus_5_pct": is_checked(values.get(columns["bonus_5_pct"])),
            "updated_at": item.get("updated_at"),
        })
    validate_popcorn_commitments(rows)
    return rows


def popcorn_snapshot(
    rows: list[dict[str, Any]],
    updated_at: str | None,
    generated_from: str,
) -> dict[str, Any]:
    committed_rows = [row for row in rows if row["commitment"] == "Committed"]
    return {
        "name": BOARDS["popcorn"]["display_name"],
        "source_name": BOARDS["popcorn"]["name"],
        "url": BOARDS["popcorn"]["url"],
        "generated_from": generated_from,
        "updated_at": updated_at,
        "items": len(rows),
        "committed": len(committed_rows),
        "participation_rate": len(committed_rows) / len(rows) if rows else None,
        "commitment_status": count_workbook_labels(rows, "commitment") if rows else [],
        "districts": count_workbook_labels(rows, "district", "Unassigned") if rows else [],
        "goal_2026": sum(row["goal_2026"] for row in rows),
        "sales_2025": sum(row["sales_2025"] for row in rows),
        "committed_goal_2026": sum(row["goal_2026"] for row in committed_rows),
        "committed_sales_2025": sum(row["sales_2025"] for row in committed_rows),
        "onboarding_11_of_11": sum(row["onboarding"] == "11/11" for row in rows),
        "onboarding_complete": sum(row["onboarding_complete"] for row in rows),
        "unit_trained": sum(row["unit_trained"] for row in rows),
        "rows": rows,
    }
    ...
    return {
        ...
        "boards": {
            ...
            "popcorn": popcorn_snapshot(
                compact_popcorn,
                latest_workbook_update(popcorn_rows),
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "popcorn": snapshot["boards"]["popcorn"]["items"],
    }, indent=2))
    return 0
