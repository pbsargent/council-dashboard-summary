...
function councilRetentionSummary() {
  const rows = (state.data?.dashboard?.unit_metric_compare || [])
    .find((section) => section.section === "All Units")?.rows || [];
...
function renderKpis() {
  ...
  const tiles = [
    ...
    ["Retention", p(retention.rate), `All Units workbook retention, weighted by ${n(retention.units)} units`, "teal"],
...
function sytReviewSummary() {
  const rows = state.data?.dashboard?.syt_people || [];
...
function renderSytPanel() {
  ...
  document.getElementById("sytPanel").innerHTML = `
    <div class="focus-metric">
      <strong>${p(state.data.dashboard.council.syt_pct)}</strong>
