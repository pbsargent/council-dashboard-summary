...
function districtServiceAreaMap() {
  return new Map((state.dashboard?.dashboard?.districts || [])
    .map((row) => [row.district, row.service_area])
    .filter(([, area]) => area));
}

function rowServiceAreas(row) {
  const map = districtServiceAreaMap();
  return [...new Set(districtLabels(rowDistrict(row)).map((label) => map.get(label)).filter(Boolean))];
}

function rowMatchesServiceArea(row, serviceArea) {
  return !serviceArea || rowServiceAreas(row).includes(serviceArea);
}
...
function officialDistrictRows(rows) {
  ...
  const areaMap = districtServiceAreaMap();
  const counts = new Map();
  for (const row of rows) {
    for (const label of districtLabels(rowDistrict(row))) {
      if (!official.has(label)) continue;
      if (serviceArea && areaMap.get(label) !== serviceArea) continue;
...
function currentRows() {
  ...
  return state.rows.filter((row) => {
    return (board === "all" || row.board === board)
      && rowMatchesServiceArea(row, serviceArea)
      && (!district || districtLabels(rowDistrict(row)).includes(district))
...
function renderControls() {
  ...
  let selectedServiceArea = serviceAreaSelect.value;
  ...
  const areaMap = districtServiceAreaMap();
  const serviceAreas = [...new Set([...areaMap.values()].filter(Boolean))]
    .sort((a, b) => a.localeCompare(b));
  if (selectedServiceArea && !serviceAreas.includes(selectedServiceArea)) selectedServiceArea = "";

  const districtRows = selectedStatus
    ? boardRows.filter((row) => rowStatus(row) === selectedStatus)
    : boardRows;
  const statusRows = selectedDistrict
    ? boardRows.filter((row) => districtLabels(rowDistrict(row)).includes(selectedDistrict))
    : boardRows;

  const districts = [...new Set(districtRows
    .filter((row) => rowMatchesServiceArea(row, selectedServiceArea))
    .map(rowDistrict)
    .filter(Boolean))]
    .sort((a, b) => a.localeCompare(b));
  const statuses = [...new Set(statusRows
    .filter((row) => rowMatchesServiceArea(row, selectedServiceArea))
    .map(rowStatus)
    .filter(Boolean))]
    .sort((a, b) => a.localeCompare(b));
  ...
  serviceAreaSelect.innerHTML = `<option value="">All service areas</option>${
    serviceAreas.map((area) => `<option value="${esc(area)}">${esc(area)}</option>`).join("")
  }`;
  ...
  serviceAreaSelect.value = selectedServiceArea;
...
function tayRollups() {
  ...
  for (const row of schoolRows) {
    for (const label of districtLabels(row.scouting_district)) {
      if (!official.has(label)) continue;
      if (serviceArea && districtServiceAreaMap().get(label) !== serviceArea) continue;
      if (district && label !== district) continue;
      if (!rollups.has(label)) {
        rollups.set(label, { district: label, service_area: districtServiceAreaMap().get(label) || "", schools: 0, tay: 0, members: memberByDistrict.get(label) || null });
      }
...
function renderTayRows() {
  ...
  document.getElementById("tayRows").innerHTML = rows.map((row) => `
    <tr>
      ...
      <td>${esc(row.service_area || "n/a")}</td>
  ...
  `).join("") || '<tr><td colspan="6"><div class="empty-state">No matching school TAY rows.</div></td></tr>';
}
...
function renderDetailRows() {
  ...
  document.getElementById("detailRows").innerHTML = rows.map((row) => `
    <tr>
      ...
      <td>${esc(rowServiceAreas(row).join(", ") || "n/a")}</td>
  ...
  `).join("") || '<tr><td colspan="9"><div class="empty-state">No matching monday.com rows.</div></td></tr>';
}
...
function bindEvents() {
  ...
  ["serviceAreaSelect", "districtSelect", "statusSelect", "searchInput"].forEach((id) => {
...
async function init() {
  const [mondayResponse, dashboardResponse] = await Promise.all([
    fetch(`data/monday-latest.json?v=${Date.now()}`, { cache: "no-store" }),
