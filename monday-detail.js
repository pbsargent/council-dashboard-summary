...
function councilTaySummary() {
  const members = ProgramFilter.isCouncil()
    ? numeric(state.dashboard?.dashboard?.council?.members)
    : (state.unitData?.units || []).filter((unit) => ProgramFilter.matchesUnitType(unit.unit_type))
      .reduce((total, unit) => total + numeric(unit.youth), 0);
...
function currentRows() {
  ...
  return state.rows.filter((row) => {
    return ProgramFilter.matchesMondayRow(row, row.board)
      && (board === "all" || row.board === board)
...
function renderKpis() {
  ...
  const tiles = [
    ...
    [ProgramFilter.isCouncil() ? "Schools" : "Eligible-span schools", n(schools.length), ProgramFilter.isCouncil() ? `${p(schoolsWithUnits / Math.max(1, schools.length))} with unit associated` : "Grade/age span overlaps selected program", "teal"],
...
function tayRollups() {
  ...
  const q = document.getElementById("searchInput").value.trim().toLowerCase();
  const official = officialDistrictSet();
  const memberByDistrict = ProgramFilter.isCouncil()
    ? new Map((state.dashboard?.dashboard?.districts || []).map((row) => [row.district, numeric(row.members)]))
    : (state.unitData?.units || []).filter((unit) => ProgramFilter.matchesUnitType(unit.unit_type)).reduce((map, unit) => {
      const districtLabel = ProgramFilter.cleanDistrict(unit.district);
      map.set(districtLabel, (map.get(districtLabel) || 0) + numeric(unit.youth));
      return map;
    }, new Map());
  const rollups = new Map();
  const schoolRows = state.rows.filter((row) => {
    return ProgramFilter.matchesMondayRow(row, "schools")
      && row.board === "schools"
...
function renderTayRows() {
  ...
  document.getElementById("tayTitle").textContent = ProgramFilter.isCouncil()
    ? "Membership as Percent of Total Available Youth"
    : `Membership as Percent of Estimated ${ProgramFilter.get()} TAY`;
  document.getElementById("tayTayHeader").textContent = ProgramFilter.tayLabel();
...
async function init() {
  const [mondayResponse, dashboardResponse, unitResponse] = await Promise.all([
